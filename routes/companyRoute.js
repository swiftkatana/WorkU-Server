...
router.post("/sendshiftsforemployee", async (req, res) => {
  ...
  let user = await User.findOne({ email })
    ...
    .then((docs) => docs || responedList.userNotFound);
  if (user.err) {
...
router.post("/uploadaudio", (req, res) => {
  ...
  upload(req, res, async function (err) {
    ...
    res.send(
      readComOrUser === "manager"
        ? company.tasks.processing[updateTask._id].audios[
            company.tasks.processing[updateTask._id].audios.length - 1
          ]
        : user.tasks.processing[updateTask._id].audios[
            company.tasks.processing[updateTask._id].audios.length - 1
          ]
    );
  });
...
router.post("/createcompany", async (req, res) => {
  ...
  newCompany.save((err) => {
    if (err) {
      looger(err.message);
      res.send(
        err.code === 11000 ? responedList.isInUse : responedList.FaildSave
      );
      return;
    } else {
      user.save((err) => {
...
router.post("/getcompany", async (req, res) => {
  ...
  const comapny = await Company.findOne(filter)
    .exec()
    .catch((err) => responedList.DBError)
    .then((doc) => doc || responedList.NotExists);
  if (comapny.err) {
...
router.delete("/leave", async (req, res, next) => {
  ...
  User.findOne({ unqiePassword: manager.unqiePassword })
    .then(async (docManager) => {
      if (!docManager) {
      ...
      if (saveCompany) {
        saveEmployee = await User.findOneAndUpdate(
          { email },
          { company: { name: "", status: "" } },
          { new: true }
        );
...
router.post("/updatepersonalreuqest", async (req, res) => {
  ...
  let user = await User.findOne({ email })
    .catch((err) => responedList.FaildSave)
    .then((doc) => doc || responedList.usersNotFound);
  if (user.err) {
    looger(user);
    res.send(user);
